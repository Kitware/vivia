...
QList<vvTrackState> vvAdapt(const vidtk::track_state& vs)
{
  ...
  if (vs.data_.get(vidtk::tracking_keys::img_objs, objs))
    ...
    for (size_t i = 0; i < objs.size(); ++i)
      ...
      const vidtk::vidtk_pixel_coord_type& img_loc = objs[i]->get_image_loc();
      const vgl_box_2d< unsigned >& bbox = objs[i]->get_bbox();
      ...
      const vgl_polygon< vidtk::float_type >& boundary = obj.get_boundary();
